...
nohup llama-server --device Vulkan1 -hf peculiar-ragdoll/Tiel-Coder-35B-A3B-GGUF-MTP:Q4_K_S -ngl 999 --n-cpu-moe 32 -c 65536 -fa 1 -ctk f16 -ctv f16 --spec-type draft-mtp --spec-draft-n-max 2 -np 1 --temp 0.6 --top-p 0.95 --top-k 20 --min_p 0.0 --presence-penalty 0 --no-mmproj --jinja > >(awk '{ print strftime("[%Y-%m-%d %H:%M:%S]"), $0; fflush() }' >> "$log_file") 2>&1 &
